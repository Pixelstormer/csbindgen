...
pub fn emit_csharp(
    ...
    const DYNDLL_MAPPING_GETTER: &str = "GetDynDllMappings";

    let static_constructor = if options.csharp_use_monomod_dyndll {
        let mappings = if options.csharp_dyndll_custom_mapping {
            format!("{DYNDLL_MAPPING_GETTER}()")
        } else {
            String::new()
        };
        format!(
        ...
        static {class_name}()
        {{
            typeof({class_name}).ResolveDynDllImports({mappings});
        }}
#pragma warning restore CS8618"
    ...
    let dyndll_mapping_getter = if options.csharp_use_monomod_dyndll
        && options.csharp_dyndll_custom_mapping
    {
        format!("        private static partial Dictionary<string, List<DynDllMapping>> {DYNDLL_MAPPING_GETTER}();")
    } else {
        String::new()
    };
    ...
    for item in methods {
        ...
        if options.csharp_use_monomod_dyndll {
            ...
            let delegate_name = build_method_delegate_name(method_name, "signature");
            ...
            method_list_string.push_str_ln(
                &format!("        public static extern {return_type} {method_prefix}{method_name}({parameters});")
            );
        }
    ...
    if options.csharp_use_monomod_dyndll {
        imported_namespaces.push_str_ln("using MonoMod.Utils;")
    }
