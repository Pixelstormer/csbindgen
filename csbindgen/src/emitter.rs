...
pub fn emit_csharp(
    ...
    const DYNDLL_MAPPINGS_GETTER: &str = "GetDynDllMappings";

    let static_constructor = if options.csharp_use_monomod_dyndll {
        let mappings = if options.csharp_dyndll_custom_mapping {
            format!("{DYNDLL_MAPPINGS_GETTER}()")
        } else {
            String::new()
        };
        format!(
            r#"#pragma warning disable CS8618 // The delegates are initialised by the `ResolveDynDllImports` method via reflection
        static {class_name}()
        {{
...
#pragma warning restore CS8618"#
    ...
    {
        format!("       private static partial Dictionary<string, List<DynDllMapping>> {DYNDLL_MAPPINGS_GETTER}();")
    } else {
        String::new()
    };
